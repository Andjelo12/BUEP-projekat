...
*/
    $(document).on('click', '.toggleBlockBtn', function(event) {
        event.preventDefault();
        var $btn = $(this);
        var email = $btn.data('email');
        var action = $btn.data('action'); // 'block' ili 'unblock'

        $.ajax({
            url: 'block_api_token.php',
            type: 'POST',
            data: {
                email: email,
                action: action
            ...
            success: function(response){
                    ...
                    if (response.status === 'success') {
                        $('#example').DataTable().draw(false); // refresuj bez gubitka strane
                    } else {
                        $('#message').text('Greška: ' + json.message);
                        $('#notificationModal').modal('show');
                    }
            },
            error: function() {
                alert('Došlo je do greške pri zahtevu.');
            }
        });
    });
